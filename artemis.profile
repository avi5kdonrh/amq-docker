...
# Java Opts
if [ -z "$JAVA_ARGS" ]; then
    JAVA_ARGS="-Djava.net.preferIPv4Stack=true   -XX:MinHeapFreeRatio=10 -XX:MaxHeapFreeRatio=20 -XX:GCTimeRatio=4 -XX:AdaptiveSizePolicyWeight=90 -XX:MaxMetaspaceSize=100m -XX:+ExitOnOutOfMemoryError -XX:+PrintClassHistogram -XX:+UseG1GC  -Dhawtio.realm=activemq  -Dhawtio.offline=true -Dhawtio.role=admin,readonly -Dhawtio.rolePrincipalClasses=org.apache.activemq.artemis.spi.core.security.jaas.RolePrincipal -Djolokia.policyLocation=${ARTEMIS_INSTANCE_ETC_URI}jolokia-access.xml -Djon.id=amq"
fi
#
